...
{
    static readonly ConcurrentDictionary<Type, IReadOnlyList<Type>> HandledMessageTypesCache = new();
    ...
    {
        Type[] GetCompatibleMessageTypes(Type _) => typeof(TMessage).GetBaseTypes().Concat(new[] { typeof(TMessage) }).Select(type => typeof(IHandleMessages<>).MakeGenericType(type)).ToArray();

        var implementedInterfaces = HandledMessageTypesCache.GetOrAdd(typeof(TMessage), GetCompatibleMessageTypes);
